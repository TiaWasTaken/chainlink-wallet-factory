...
import TokenSwap from "./actions/TokenSwap";
...
export default function EtherMenu({ account, setActiveWallet }) {
  ...
  const renderSection = () => {
    switch (selected) {
      ...
      case "swap":
        return <TokenSwap account={account} />;
      case "gas":
        return <GasTracker />;
      case "history":
  ...
  return (
    <section
      className={`relative w-full min-h-[700px] flex flex-col items-center text-white overflow-visible ${selected ? "pt-24 pb-40" : "pt-24 pb-32"
        }`}
    >
      <motion.div
