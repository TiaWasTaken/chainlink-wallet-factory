...
export default function SwapSection() {
  ...
  const swap = new ethers.Contract(
    ...
    EthUsdcSwapABI.abi,      // 👈 usa .abi
    signer
  );
  const usdc = new ethers.Contract(
    addresses.USDCMock,
    USDCMockABI.abi,         // 👈 usa .abi
    signer
  );
  const agg = new ethers.Contract(
    addresses.MockV3Aggregator,
    MockV3AggregatorABI.abi, // 👈 usa .abi
    provider
  );
  ...
  const fetchPrice = async () => {
    ...
    try {
      ...
      if (decimals === 8) {
        ...
        normalized = Number(
          answer / BigInt(10 ** (decimals - 8))
        );
      } else {
        normalized = Number(
          answer * BigInt(10 ** (8 - decimals))
        );
      }
  ...
  }, [aggContract, account]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleBuyUsdc = async () => {
    ...
    try {
      ...
      const tx = await swapContract.buyUsdc(account, { value });
  ...
  const handleSellUsdc = async () => {
    ...
    try {
      ...
      // Sell
      const tx = await swapContract.sellUsdc(account, amount);
      const receipt = await tx.wait();
